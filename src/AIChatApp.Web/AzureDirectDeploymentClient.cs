...
{
    public static string ConvertUri(string originalUri)
    {
        if (string.IsNullOrEmpty(originalUri))
        {
            throw new ArgumentException("The URI cannot be null or empty.", nameof(originalUri));
        }

        // Use a regular expression to remove the /openai/deployments/{deployment-name} part
        string convertedUri = Regex.Replace(originalUri, "/openai/deployments/[^/]*", "");

        // Remove the query string if it exists
        int queryIndex = convertedUri.IndexOf('?');
        if (queryIndex >= 0)
        {
            convertedUri = convertedUri.Substring(0, queryIndex);
        }
